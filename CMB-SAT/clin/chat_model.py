...
class ChatModel(nn.Module, GenerationMixin):
    ...
    @torch.no_grad()
    def chat(self, tokenizer, query: str, history: List[Tuple[str, str]] = None, max_length: int = 4096, num_beams=1,
             do_sample=True, top_p=0.75, temperature=0.1, logits_processor=None, **kwargs):
        if history is None:
            history = []
    ...
    @torch.no_grad()
    def batch_generate(self, tokenizer, queries, max_length: int = 4096, num_beams=1, do_sample=True, top_p=0.75, temperature=0.1, **kwargs):
